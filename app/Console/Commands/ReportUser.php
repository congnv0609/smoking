...
class ReportUser extends Command
{
    ...
    public function UpdateSmoker($accountId)
    {
        ...
        $data['future_ema'] = $this->countFutureEma($accountId);
    ...
    public function countFutureEma($accountId)
    {
        $count = 0;
        $count += Ema1::where([['popup_time', '>', new DateTime()], ['account_id', $accountId]])->count();
        $count += Ema2::where([['popup_time', '>', new DateTime()], ['account_id', $accountId]])->count();
        $count += Ema3::where([['popup_time', '>', new DateTime()], ['account_id', $accountId]])->count();
        $count += Ema4::where([['popup_time', '>', new DateTime()], ['account_id', $accountId]])->count();
        $count += Ema5::where([['popup_time', '>', new DateTime()], ['account_id', $accountId]])->count();
        return $count;
    }
}
