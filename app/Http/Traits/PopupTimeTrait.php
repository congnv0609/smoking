...
trait PopupTimeTrait {
        ...
        foreach ($data as $key => $value) {
            if (!empty($value)) {
                ...
                if ($end_time > (new DateTime())) {
                    ...
                    $curEma->ema = $key+1;
                    $curEma->nth_popup = $curEma->nth_popup < 3 ? $curEma->nth_popup + 1 : 3;
                    return $curEma;
                }
            }
        }
    ...
    {
        return Ema1::select('id', 'account_id', 'date', 'nth_day', 'nth_popup', 'popup_time', 'popup_time1', 'popup_time2', 'postponded_1', 'postponded_2', 'postponded_3')->where('account_id', $accountId)
            ->where('date', '>=', date_format(new DateTime(), 'Y-m-d'))
            ->first();
    }
    private function getPopupTimeEma2($accountId)
    {
        return Ema2::select('id', 'account_id', 'date', 'nth_day', 'nth_popup', 'popup_time', 'popup_time1', 'popup_time2', 'postponded_1', 'postponded_2', 'postponded_3')->where('account_id', $accountId)
            ->where('date', '>=', date_format(new DateTime(), 'Y-m-d'))
            ->first();
    }
    private function getPopupTimeEma3($accountId)
    {
        return Ema3::select('id', 'account_id', 'date', 'nth_day', 'nth_popup', 'popup_time', 'popup_time1', 'popup_time2', 'postponded_1', 'postponded_2', 'postponded_3')->where('account_id', $accountId)
            ->where('date', '>=', date_format(new DateTime(), 'Y-m-d'))
            ->first();
    }
    private function getPopupTimeEma4($accountId)
    {
        return Ema4::select('id', 'account_id', 'date', 'nth_day', 'nth_popup', 'popup_time', 'popup_time1', 'popup_time2', 'postponded_1', 'postponded_2', 'postponded_3')->where('account_id', $accountId)
            ->where('date', '>=', date_format(new DateTime(), 'Y-m-d'))
            ->first();
    }
    private function getPopupTimeEma5($accountId)
    {
        return Ema5::select('id', 'account_id', 'date', 'nth_day', 'nth_popup', 'popup_time', 'popup_time1', 'popup_time2', 'postponded_1', 'postponded_2', 'postponded_3')->where('account_id', $accountId)
            ->where('date', '>=', date_format(new DateTime(), 'Y-m-d'))
            ->first();
    }
}